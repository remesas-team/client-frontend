        ...
        <div class="bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="font-medium">1 {{ remittance.source_currency_symbol }} = {{ Number(remittance.exchange_rate).toFixed(2) }} {{ remittance.destination_currency_symbol }}</span>
            </div>

            <div class="flex justify-between items-center">
              <span class="text-gray-600">Comisión</span>
              <span class="font-medium">{{ remittance.source_currency_symbol }} {{ Number(remittance.source_commission_fee).toFixed(2) }}</span>
            </div>

            <div class="flex justify-between items-center">
              <span class="text-gray-600">Impuestos</span>
              <span class="font-medium">{{ remittance.source_currency_symbol }} {{ Number(remittance.source_tax).toFixed(2) }}</span>
            </div>

            <div class="flex justify-between items-center font-bold text-lg">
              <span>Monto a recibir</span>
              <span>{{ remittance.destination_currency_symbol }} {{ Number(remittance.destination_amount).toFixed(2) }}</span>
            </div>
          </div>
